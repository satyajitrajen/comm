...
- 2026-09-10: Production readiness hardening (branch `feat/production-readiness`, plan in `docs/superpowers/plans/2026-09-10-production-readiness.md`). Backend: strict DTO validation (`whitelist` + `forbidNonWhitelisted`) across chats/messages/files/auth/calendar/tasks/users/admin; upload MIME allowlist (blocks SVG/HTML); password-reset tokens stored as SHA-256 hashes with O(1) lookup; OTP via crypto-random; `assertActiveLoginSession` deny-by-default; localhost origins removed from CORS. Datastore: Prisma switched to PostgreSQL 16 with baseline migration, SQLite migrations archived to `prisma/migrations-sqlite-archive/`, one-off `prisma/migrate-sqlite-data.ts` carry-over script; entrypoint/deploy scripts use `migrate deploy`; compose adds postgres+redis with healthchecks. Realtime: optional `@socket.io/redis-adapter` when `REDIS_URL` set; presence/activeCalls remain per-process — default deployment is a single backend instance. Perf: chats feed unread counts via single `groupBy`, read receipts via bulk `createMany(skipDuplicates)`, new indexes on Conversation/ConversationParticipant/WorkspaceUser/Message/File/AuditLog. Mobile: authenticated disk-cached images (`AuthImage` via `cached_network_image` with bearer header; viewer fetches bytes via Dio; `TtAvatar` loads absolute URLs only so tokens never reach third-party hosts); downloads via `teamtime/downloads` MethodChannel → MediaStore IS_PENDING flow (API 29+) / legacy Downloads + runtime permission (26-28); release signing via `android/key.properties` with debug-signed fallback (NOT publishable without keystore — release-ops task); minify + resource shrink with Jitsi/WebRTC/socket keep rules; socket reconnect capped (10 attempts, backoff+jitter; server-disconnect retries capped at 5) and dev API override restricted to debug builds; tabs converted to `StatefulShellRoute.indexedStack`; call flags precomputed per mutation (no per-build history scan); 45s incoming ring auto-timeout; notification Accept/Decline wired to the call controller (decline emits `call.decline`; background accept routes into the conversation's join banner); mic+camera requested before joining with denial SnackBar.
...
- Runtime Postgres/Redis verification (migrations + data carry-over + multi-tab socket behavior) must run on the deploy box; Docker is not available on the dev machine.
- Release APK is debug-signed until a Play keystore exists and `android/key.properties` is created (see `key.properties.example`).
