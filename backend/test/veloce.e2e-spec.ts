...
describe('Veloce E2E Core Integration Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    await prismaService.$executeRawUnsafe('DELETE FROM User');
    await prismaService.$executeRawUnsafe('DELETE FROM Workspace');
    await prismaService.$executeRawUnsafe('DELETE FROM Conversation');
  });
